...
=head1 AUTHOR
 ...
 and further to this release (CC4) by Jelena Telenius March 2016
...
my $report2_filename = "$sample\_$version/".$1."_report2_$version.txt";
my $report3_filename = "$sample\_$version/".$1."_report3_$version.txt";
my $report4_filename = "$sample\_$version/".$1."_report4_$version.txt";
  ...
  #if (($chr =~ /chr(.*)/) and ($cigar =~/(\d++)(.*)/) and ($name =~ /(.*):PE(\d++):(\d++)$/)) # checks that the sam file matches the name, cigar string etc (non aligned reads will not do this)
  {
    ...
    # The rest of the reads are now parse-able :
		...
		if (($NEWstart-2)==$data{$readname}{$pe}{$readno}{"readstart"}) {
	          push @{$data{$readname}{"coord array"}}, $data{$readname}{$pe}{$readno}{"chr"}.":".$data{$readname}{$pe}{$readno}{"readstart"}."-".$NEWend."-".$data{$readname}{$pe}{$readno}{"strand"};		  
		  ...
		  push @{$data{$readname}{"coord array"}}, $NEWstart."-".$data{$readname}{$pe}{$readno}{"readend"}."-".$data{$readname}{$pe}{$readno}{"strand"};
		  $nonflashedcounters{"02 Fragment has RestrictionEnzyme cut on RIGHT side (forward coordinates)"}++;
		}
		else {
		  push @{$data{$readname}{"coord array"}}, $NEWstart."-".$NEWend."-".$data{$readname}{$pe}{$readno}{"strand"};
